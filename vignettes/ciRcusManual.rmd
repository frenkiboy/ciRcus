...
The package can be currently installed from BIMSBbioinfo github page:
```{r install_ciRcus, eval=FALSE, echo=TRUE}
install.packages('devltools')
devtools::install_github('BIMSBbioinfo/ciRcus')
```


# Circ RNA detection

Currently there are multiple tools which enable detection of circular RNAs
from RNAseq data.
...
The SummarizedExperiment object contains three matrices:
1. circ: backsplicing reads
2. linear.start: linearly spliced reads over the circ start 
3. linear.start: linearly spliced reads over the circ end

To access the counts do the following:
```{r Assays_Circs, eval=FALSE, echo=TRUE}
